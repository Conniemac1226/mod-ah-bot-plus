...
- When `mod-individual-progression` is active, the bot treats the server progression limit as the live market stage, so Launch pricing is used at classic launch and the pricing curve advances automatically as you unlock later tiers.
- Pricing research can live in `modules/mod_ahbot_pricing.csv` via `AuctionHouseBot.PricingCsvFile`. The loader reads `option,value` rows after the normal config, and list-style values use `;` inside the CSV cell so the file stays two-column.
- The pricing CSV can also carry progression-aware unlocks via `AuctionHouseBot.Progression.MinimumStateByItemID`. Use `itemID:progressionState` pairs to keep later-expansion items hidden until the module advances far enough to support them.
